...
public class LavaBoilerFuelWidget extends Widget {
    private int fuel = 0, maxFuel = 0;
    protected LavaBoilerFuelWidget(@Nonnull GuiInstance gui, @Nullable IGuiElement parent) {
        super(gui, parent);
    }

    public static WidgetSupplier build() {
        return builder(LavaBoilerFuelWidget::new);
    }

    @Override
    public void init() {
        super.init();
        gui.syncInt(() -> ((TileEntityLavaBoiler)((ContainerMachine<?>)gui.container).getTile()).getFuel(), i -> fuel = i, SERVER_TO_CLIENT);
        gui.syncInt(() -> ((TileEntityLavaBoiler)((ContainerMachine<?>)gui.container).getTile()).getMaxFuel(), i -> maxFuel = i, SERVER_TO_CLIENT);
    }
