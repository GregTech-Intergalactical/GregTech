...
public class WasherLoader {
    public static void init() {
        AntimatterMaterialTypes.CRUSHED.all().forEach(m -> {
            ...
            ORE_WASHING.RB().fi(AntimatterMaterials.Water.getLiquid(1000)).ii(RecipeIngredient.of(AntimatterMaterialTypes.CRUSHED.get(m,1))).io(AntimatterMaterialTypes.CRUSHED_PURIFIED.get(m,1), AntimatterMaterialTypes.DUST_TINY.get(aOreByProduct1,1), stoneDust).add("crushed_" + m.getId(),200, 24);
        });
    }
}
