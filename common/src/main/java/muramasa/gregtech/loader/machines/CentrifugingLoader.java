...
public class CentrifugingLoader {
    public static void init() {
        AntimatterMaterialTypes.DUST_IMPURE.all().forEach(dust -> {
            if (dust.hasByProducts()) CENTRIFUGING.RB().ii(of(AntimatterMaterialTypes.DUST_IMPURE.get(dust),1)).io(new ItemStack(AntimatterMaterialTypes.DUST.get(dust), 1), AntimatterMaterialTypes.DUST_TINY.get(dust.getByProducts().get(0), 1)).add("dust_impure_" + dust.getId(), 400, 2);
            else CENTRIFUGING.RB().ii(of(AntimatterMaterialTypes.DUST_IMPURE.get(dust),1)).io(new ItemStack(AntimatterMaterialTypes.DUST.get(dust), 1)).chances(1.0, 0.1).add("dust_impure_" + dust.getId(),dust.getMass(), 2);
        });
        AntimatterMaterialTypes.DUST_PURE.all().forEach(dust -> {
            if (dust.hasByProducts())  {
                int index = dust.getByProducts().size() > 1 ? 1 : 0;
                CENTRIFUGING.RB().ii(of(AntimatterMaterialTypes.DUST_PURE.get(dust),1)).io(new ItemStack(AntimatterMaterialTypes.DUST.get(dust), 1), AntimatterMaterialTypes.DUST_TINY.get(dust.getByProducts().get(index), 1)).add("dust_pure_" + dust.getId(),dust.getMass(), 2);
            }
            else CENTRIFUGING.RB().ii(of(AntimatterMaterialTypes.DUST_IMPURE.get(dust),1)).io(new ItemStack(AntimatterMaterialTypes.DUST.get(dust), 1)).chances(1.0, 0.1).add("dust_pure_" + dust.getId(),dust.getMass(), 2);
        });
        ...
        CENTRIFUGING.RB().ii(RecipeIngredient.of(AntimatterMaterialTypes.DUST.get(AntimatterMaterials.Stone, 32))).io(AntimatterMaterialTypes.DUST.get(AntimatterMaterials.Quartz, 9), AntimatterMaterialTypes.DUST.get(Materials.PotassiumFeldspar, 9), AntimatterMaterialTypes.DUST.get(Materials.Marble, 8), AntimatterMaterialTypes.DUST.get(Materials.Biotite, 4),
                AntimatterMaterialTypes.DUST.get(Materials.Sodalite, 4)).add("stone_dust",7680, 30);
    }
}
