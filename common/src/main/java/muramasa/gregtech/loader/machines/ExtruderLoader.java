...
public class ExtruderLoader {
    public static void init() {
        AntimatterMaterialTypes.RING.all().forEach(r -> {
            ...
            EXTRUDING.RB().ii(of(AntimatterMaterialTypes.INGOT.getMaterialTag(r),1),of(GTCoreItems.ShapeRing,1).setNoConsume()).io(AntimatterMaterialTypes.RING.get(r,4)).add("ring_" + r.getId(),duration,30);
        });
        ...
        AntimatterMaterialTypes.GEAR.all().forEach(g -> {
            ...
            EXTRUDING.RB().ii(of(AntimatterMaterialTypes.INGOT.getMaterialTag(g),4),of(GTCoreItems.ShapeGear,1).setNoConsume()).io(AntimatterMaterialTypes.GEAR.get(g,1)).add("gear_" + g.getId(),duration*4,30);
        });
        ...
        AntimatterMaterialTypes.BOLT.all().forEach(b -> {
            ...
            EXTRUDING.RB().ii(of(AntimatterMaterialTypes.INGOT.getMaterialTag(b),1),of(GTCoreItems.ShapeBolt,1).setNoConsume()).io(AntimatterMaterialTypes.BOLT.get(b,8)).add("bolt_" + b.getId(),duration,30);
        });

        AntimatterMaterialTypes.GEAR_SMALL.all().forEach(g -> {
            if (!g.has(AntimatterMaterialTypes.INGOT)) return;
            long duration = g.getElement() == null ? Math.max(g.getMass(), 1) : g.getElement().getHardness();
            EXTRUDING.RB().ii(of(AntimatterMaterialTypes.INGOT.getMaterialTag(g),1),of(GTCoreItems.ShapeGearSmall,1).setNoConsume()).io(AntimatterMaterialTypes.GEAR_SMALL.get(g,1)).add("gear_small_" + g.getId(),duration,30);
        });
        ...
        AntimatterMaterialTypes.PLATE.all().forEach(p -> {
            ...
            EXTRUDING.RB().ii(of(AntimatterMaterialTypes.INGOT.getMaterialTag(p),1),of(GTCoreItems.ShapePlate,1).setNoConsume()).io(AntimatterMaterialTypes.PLATE.get(p,1)).add("plate_" + p.getId(),duration,30);
        });

        AntimatterMaterialTypes.ROD.all().forEach(r -> {
            if (!r.has(AntimatterMaterialTypes.INGOT)) return;
            long duration = r.getElement() == null ? Math.max(r.getMass(), 1) : r.getElement().getHardness();
            EXTRUDING.RB().ii(of(AntimatterMaterialTypes.INGOT.getMaterialTag(r),1),of(GTCoreItems.ShapeRod,1).setNoConsume()).io(AntimatterMaterialTypes.ROD.get(r,2)).add("rod_" + r.getId(),duration,30);
        });
        ...
        AntimatterAPI.all(Wire.class).forEach(t -> {
            ...
            EXTRUDING.RB().ii(of(AntimatterMaterialTypes.INGOT.getMaterialTag(t.getMaterial()),1),of(GTCoreItems.ShapeWire,1).setNoConsume()).io(stack).add("wire_" + t.getMaterial().getId(),duration,30);
        });
