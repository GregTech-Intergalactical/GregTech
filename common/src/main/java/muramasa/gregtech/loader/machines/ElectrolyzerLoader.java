...
public class ElectrolyzerLoader {
    public static void init() {
        AntimatterMaterialTypes.DUST.all().stream().filter(t -> t.has(ELEC)).forEach(t -> {
            FluidHolder[] fluids = t.getProcessInto().stream().filter(mat -> ((mat.m.has(AntimatterMaterialTypes.GAS) || mat.m.has(AntimatterMaterialTypes.LIQUID)) && !mat.m.has(AntimatterMaterialTypes.DUST))).map(mat -> mat.m.has(AntimatterMaterialTypes.GAS) ? mat.m.getGas(mat.s*1000) : mat.m.getLiquid(mat.s*1000)).toArray(FluidHolder[]::new);
