...
public class LatheLoader {
    public static void init() {
        ...
        LENS.all().stream().filter(m -> m.has(DUST)).forEach(m -> {
            if (m.has(PLATE)){
                RecipeMaps.LATHE.RB().ii(PLATE.getMaterialIngredient(m, 1)).io(LENS.get(m, 1), DUST_SMALL.get(m, 1)).add(m.getId() + "_lens", Math.max(m.getMass() / 2, 1), 24);
            }
            if (m.has(GEM_EXQUISITE)){
                RecipeMaps.LATHE.RB().ii(GEM_EXQUISITE.getMaterialIngredient(m, 1)).io(LENS.get(m, 1), DUST.get(m, 2)).add(m.getId() + "_lens_exquisite", Math.max(m.getMass(), 1), 24);
            }
        });
